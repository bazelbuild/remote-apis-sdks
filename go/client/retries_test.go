...
type flakyBatchUpdateServer struct {
	numErrors int // A counter of errors the server has returned thus far.
	requests  []*repb.BatchUpdateBlobsRequest
}

func (f *flakyBatchUpdateServer) FindMissingBlobs(ctx context.Context, req *repb.FindMissingBlobsRequest) (*repb.FindMissingBlobsResponse, error) {
	return nil, status.Error(codes.Unimplemented, "")
}

func (f *flakyBatchUpdateServer) BatchReadBlobs(ctx context.Context, req *repb.BatchReadBlobsRequest) (*repb.BatchReadBlobsResponse, error) {
	return nil, status.Error(codes.Unimplemented, "")
}

func (f *flakyBatchUpdateServer) GetTree(req *repb.GetTreeRequest, stream regrpc.ContentAddressableStorage_GetTreeServer) error {
	return status.Error(codes.Unimplemented, "")
}

func (f *flakyBatchUpdateServer) BatchUpdateBlobs(ctx context.Context, req *repb.BatchUpdateBlobsRequest) (*repb.BatchUpdateBlobsResponse, error) {
...
func TestBatchUpdateBlobsIndividualRequestRetries(t *testing.T) {
	...
	if len(fake.requests) != len(wantRequests) {
		t.Errorf("client.BatchWriteBlobs(ctx, blobs) wrong number of requests; expected %d, got %d", len(wantRequests), len(fake.requests))
	}
	for i, req := range wantRequests {
		if diff := cmp.Diff(req, fake.requests[i], cmpopts.SortSlices(func(a, b interface{}) bool {
